...
import QrCodeGenerator from "src/components/manual/qr-code-generator";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export const columns: ColumnDef<TTable>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <QrCodeGenerator
        textContent={`Mã bàn: ${row.getValue("number")}`}
        value={getTableLink({ token: row.getValue("token"), tableNumber: row.getValue("number") })}
      />
    ),
  },
  {
